// scripts/convert-pizza-express-to-draft.ts
...
async function convertToDraft() {
  // Find the Pizza Express Dough Balls recipe
  const recipe = await client.fetch(
    `*[_type == "recipe" && title match "*Pizza Express*" && title match "*Dough Balls*"][0]`
  );

  if (!recipe) {
    console.log("❌ Pizza Express Dough Balls recipe not found!");
    return;
  }
  ...
  console.log("2. Edit the draft to add details");
