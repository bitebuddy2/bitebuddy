...
export default function MealPlannerCalendar() {
  ...
  function exportToPDF() {
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      const startY = page === 0 ? 36 : 26;
      const colWidth = contentWidth / numDaysOnPage;
      const rowHeight = 24; // Further reduced to fit all content with footer
      ...
      doc.text('Notes', margin + 3, currentY + 4.5);
      currentY += 7;
      ...
      pageDays.forEach((day, index) => {
        ...
        doc.roundedRect(x + 1, currentY, colWidth - 2, 10, 2, 2);

        if (dayNote?.note) {
          doc.setFillColor(254, 249, 195); // yellow-100
          doc.roundedRect(x + 2, currentY + 1, colWidth - 4, 8, 2, 2, 'F');
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(6);
          doc.setTextColor(0, 0, 0);
          ...
          noteLines.slice(0, 2).forEach((line: string, lineIndex: number) => {
            doc.text(line, x + 3, currentY + 3.5 + (lineIndex * 2.5));
          });
        } else {
          doc.setFontSize(6);
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(156, 163, 175);
          doc.text('-', x + (colWidth / 2), currentY + 6, { align: 'center' });
          doc.setTextColor(0, 0, 0);
        }
      });
      ...
      mealTypes.forEach((mealType) => {
        ...
        doc.roundedRect(margin, currentY, contentWidth, 6, 1, 1, 'F');
        doc.text(mealType.charAt(0).toUpperCase() + mealType.slice(1), margin + 3, currentY + 4.5);
        ...
        pageDays.forEach((day, index) => {
          ...
          if (recipe) {
            ...
            const maxLines = 3;
            const startTextY = currentY + 4.5;

            lines.slice(0, maxLines).forEach((line: string, lineIndex: number) => {
              doc.text(line, x + 4, startTextY + (lineIndex * 2.8));
            });
      ...
      // Footer - ensure it's below all content
      doc.setFontSize(7);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(156, 163, 175); // gray-400
      // Use the greater of: fixed position or below content
      const footerY = Math.max(currentY + 3, pageHeight - 8);
